-- Gauf Database Export
-- Generated on: 2025-09-04 13:22:00
-- Database: kurssienhallinta
-- 
-- This file contains the complete database structure and data
-- Import this file to recreate the database on another server
-- 
...
DROP TABLE IF EXISTS `kurssikirjautumiset`;
...
DROP VIEW IF EXISTS `kurssi_nakyma`;
...
DROP VIEW IF EXISTS `opettaja_nakyma`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `opettaja_nakyma` AS select `o`.`tunnus` AS `tunnus`,`o`.`etunimi` AS `etunimi`,`o`.`sukunimi` AS `sukunimi`,`o`.`aine` AS `aine`,count(`k`.`tunnus`) AS `kurssien_maara` from (`opettajat` `o` left join `kurssit` `k` on(`o`.`tunnus` = `k`.`opettaja_tunnus`)) group by `o`.`tunnus`;

-- --------------------------------------------------------
-- View structure for view `opiskelija_nakyma`
-- --------------------------------------------------------

DROP VIEW IF EXISTS `opiskelija_nakyma`;
...
DROP VIEW IF EXISTS `tila_nakyma`;
